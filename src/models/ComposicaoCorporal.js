...
module.exports = {
  ...
  async updateByTeste(idTeste, composicaoCorporal) {
    const result = await connection("composicaoCorporal").where({ idTeste }).update(composicaoCorporal);
    return result;
  },
  ...
  async verificaIdTesteExiste(idTeste){
    const result = await connection("composicaoCorporal").where({ idTeste }).select("*").first();
